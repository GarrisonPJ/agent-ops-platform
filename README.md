...
AI Agent 基础设施平台——从任务提交到轨迹回放、评测排名、训练数据导出的**完整工具链**。打通"外部工具 → Agent 执行 → 评测反馈 → RL 训练体系"的链路。
...
│  ┌──────────┐ ┌────────────┐ ┌──────────┐ ┌──────────┐ ┌───────────┐  │
│  │ Runtime  │ │ Trajectory │ │   LLM    │ │ Scoring  │ │  Export   │  │
│  │ ReAct    │ │   Repo     │ │ Adapter  │ │ Engine   │ │  Layer    │  │
│  │ 循环     │ │  持久化     │ │          │ │ 评分引擎  │ │ 训练数据   │  │
│  └────┬─────┘ └─────┬──────┘ └────┬─────┘ └────┬─────┘ └─────┬─────┘  │
│       │             │             │            │              │         │
│  ┌────▼─────┐       │        ┌────▼─────┐      │              │         │
...
| 层 | 技术 |
|----|------|
| **前端** | React 19, TypeScript, Vite 6, Tailwind v4, Redux Toolkit, RTK Query, TanStack Table, framer-motion, Lucide React, react-json-view-lite, react-resizable-panels |
...
│   │   ├── config.py            # 配置管理
│   │   ├── database.py          # 异步数据库连接
│   │   ├── models.py            # SQLAlchemy ORM（trajectories + steps）
│   │   └── adapters/            # LangChain + OpenAI Agents SDK 适配器
│   ├── run_mcp.py               # MCP Server 独立入口
│   └── tests/                   # pytest（48 测试用例，6 个测试文件）
├── frontend/
│   ├── src/
│   │   ├── pages/
...
│   │   │   └── EvalPage.tsx      # 评测面板（Benchmark + Ranking）
│   │   ├── components/          # StatusBadge, StepCard, TokenDashboard 等
...
└── .scratch/                     # PRD + Issue 追踪（v0.1 / v0.2 / v0.3）
```
...
- **API Key：** 通过 `kubectl edit secret agentops-secret -n agentops` 或直接修改 `secret.yaml` 中的 `LLM_API_KEY`
...
| Method | Path | 说明 |
|--------|------|------|
| POST | `/api/agents/run` | 提交 Agent 任务，返回 trajectory_id |
